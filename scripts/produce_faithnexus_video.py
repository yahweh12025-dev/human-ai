...
async def produce_video(scripture: Dict, out_root: Path) -> Optional[Dict]:
    ...
    # ── Delete local file after successful GDrive upload ─────────────────────
    if gdrive_ok:
        try:
            output_mp4.unlink()
            print(f"   Local file deleted: {output_mp4.name}")
        except OSError as _e:
            print(f"   Warning: could not delete local file: {_e}", file=sys.stderr)
    christian_dest = output_mp4  # keep metadata reference pointing to original path
