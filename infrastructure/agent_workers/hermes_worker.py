#!/usr/bin/env python3
"""
Hermes Worker - Processes messages from Hermes' incoming directory
...
from hermes_tools import delegate_task, terminal, send_message, memory
...
class HermesWorker:
    ...
    def process_message(self, message_file):
        ...
        try:
            ...
            elif "task" in content.lower() or "goal" in content.lower():
    ...
    def review_and_improve_repo(self, content, from_agent):
        ...
        try:
            # Use delegate_task to spawn a subagent for code review
            result = delegate_task(
                ...
                Focus areas: code quality, documentation, testing, organization, performance
                """,
                toolsets=["terminal", "file", "web", "skills"],
                role="orchestrator"
            )
            
            return f"Review completed by Hermes agent. Results: {str(result)[:500]}..."
        except Exception as e:
            return f"Error during review: {str(e)}"
            
    def assign_and_execute_task(self, content, from_agent):
        """Assign and execute a specific task"""
        try:
            # Execute task directly or delegate
            result = delegate_task(
                goal=content,
                context=f"""
                Task assigned by: {from_agent}
                Repository: {WORK_DIR}
                """,
            ...
            return f"Task executed by Hermes agent. Result: {str(result)[:500]}..."
        except Exception as e:
    ...
    def handle_general_inquiry(self, content, from_agent):
        ...
        try:
            # Use Hermes' own capabilities to respond
            response = f"""Hermes agent received message from {from_agent}:
"{content}"
...
I am ready to assist with repository improvements, code reviews, or task execution."""
