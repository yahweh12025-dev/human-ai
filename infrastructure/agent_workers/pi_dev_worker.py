#!/usr/bin/env python3
"""
Pi.dev Worker - Processes messages from pi.dev's incoming directory
...
class PiDevWorker:
    ...
    def process_message(self, message_file):
        ...
        try:
            ...
            elif "seed" in content.lower() or "profile" in content.lower() or "login" in content.lower():
                # This is related to profile seeding/login tasks
                result = self.handle_seeding_task(content, from_agent)
            else:
    ...
    def review_and_improve_repo(self, content, from_agent):
        ...
        try:
            # Use delegate_task to spawn a subagent for general review
            result = delegate_task(
                goal=f"Review the human-ai repository for improvements based on this request: {content}",
                context=f"""
                Repository: {WORK_DIR}
                Requesting agent: {from_agent}
                Current time: {datetime.utcnow().isoformat()}
                Focus areas: general code quality, documentation, testing, organization
                """,
                toolsets=["terminal", "file", "web", "skills"],
                role="orchestrator"
            )
            
            return f"Pi.dev review completed. Results: {str(result)[:500]}..."
        except Exception as e:
            return f"Error during review: {str(e)}"
            
    def handle_seeding_task(self, content, from_agent):
        """Handle profile seeding/login related tasks"""
        try:
            # Check if we have masterseed_py or similar seeding tools
            seed_tools = list(self.work_dir.glob("*masterseed*")) + list(self.work_dir.glob("*seed*"))
            
            response = f"""Pi.dev agent received seeding/login related request from {from_agent}:
"{content}"

Available seeding tools: {[str(tool.name) for tool in seed_tools[:5]]}

I can assist with:
- Profile seeding for various platforms (Claude, Perplexity, DeepSeek, Gemini)
- Login persistence configurations
- Identity management
- API key handling

Please provide more specific details about what seeding task you'd like me to perform."""
            
            return response
        except Exception as e:
            return f"Error handling seeding task: {str(e)}"
            
    def handle_general_inquiry(self, content, from_agent):
        """Handle general inquiries"""
        try:
            response = f"""Pi.dev agent received message from {from_agent}:
"{content}"
...
I am ready to assist with profile seeding, identity management, login persistence, and general repository improvements."""
