#!/usr/bin/env python3
"""
Opencode Worker - Processes messages from Opencode's incoming directory
...
class OpencodeWorker:
    ...
    def process_message(self, message_file):
        ...
        try:
            ...
            elif "task" in content.lower() or "goal" in content.lower() or "code" in content.lower():
                # This is a task assignment (especially coding tasks)
                result = self.assign_and_execute_task(content, from_agent)
            else:
    ...
    def review_and_improve_repo(self, content, from_agent):
        """Review the repository and suggest improvements using opencode"""
        try:
            # Use opencode CLI to review and improve code
            cmd = f"opencode run \"Review the human-ai repository for improvements based on this request: {content}. Focus on code quality, best practices, and implement suggested improvements.\""
            result = terminal(cmd, workdir=WORK_DIR, timeout=120)
            
            return f"Opencode review completed. Output: {result.get('output', '')[:500]}..."
        except Exception as e:
            return f"Error during opencode review: {str(e)}"
            
    def assign_and_execute_task(self, content, from_agent):
        """Assign and execute a specific task using opencode"""
        try:
            # Use opencode CLI to execute coding tasks
            cmd = f'opencode run "{content}"'
            result = terminal(cmd, workdir=WORK_DIR, timeout=180)
            
            return f"Opencode task executed. Output: {result.get('output', '')[:500]}..."
        except Exception as e:
            return f"Error executing opencode task: {str(e)}"
            
    def handle_general_inquiry(self, content, from_agent):
        """Handle general inquiries"""
        try:
            response = f"""Opencode agent received message from {from_agent}:
"{content}"
...
I am ready to assist with coding tasks, code reviews, and software development improvements."""
