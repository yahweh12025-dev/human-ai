...
class HybridLLMRouter:
    ...
    async def _should_use_model(self, task: str) -> str:
        ...
        scores = [('gemini', gemini), ('deepseek', deepseek), ('perplexity', perplexity), ('claude', claude), ('opencode', opencode), ('anythingllm', 0.5)] # Default score for anythingllm
        return max(scores, key=lambda x: x[1])[0]
    ...
    async def route_task(self, task: str, context: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            elif selected_model == "anythingllm":
                print(f"🔀 Routing to ANYTHINGLLM: {full_task[:50]}...")
                from core.utils.anything_llm_bridge import AnythingLLMBridge
                bridge = AnythingLLMBridge()
                response = await bridge.prompt(full_task)
