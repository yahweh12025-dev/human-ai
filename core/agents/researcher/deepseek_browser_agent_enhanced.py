...
class DeepSeekBrowserAgentEnhanced:
    ...
    async def login(self):
        ...
        try:
            ...
            # Using a very specific selector to avoid the login fields
            chat_input_selector = 'textarea[placeholder*="Message"], [role="textbox"]:not([type="password"])'
            
            try:
                await self.page.wait_for_selector(chat_input_selector, timeout=8000)
                print("✅ Already logged into DeepSeek (session restored)")
                return True
            except:
                print("⚠️ Not logged in or on login page. Attempting login...")
                
                # If we are on the login page, look for the Google SSO button
                google_btn_selector = 'button:has-text("Google"), a:has-text("Google"), [aria-label*="Google"]'
                try:
                    await self.page.wait_for_selector(google_btn_selector, timeout=15000)
                    await self.page.click(google_btn_selector)
                    print("🖱️ Clicked Google SSO button. Waiting for account selection...")
                    
                    # Wait for chat input to appear after login
                    await self.page.wait_for_selector(chat_input_selector, timeout=45000)
                    print("✅ Login successful via Google SSO!")
                    return True
                except Exception as sso_e:
                    print(f"❌ Google SSO attempt failed: {sso_e}")
                    print("💡 TIP: If you are running headlessly, you MUST log in manually via a GUI session to save your cookies.")
                    return False
    ...
    async def prompt(self, prompt_text: str) -> str:
        ...
        Uses robust semantic selectors and navigation verification.
        """
        if not self.is_initialized:
            await self.start_browser()
            
        # 1. Ensure we are on the correct page and logged in
        if not await self.login():
            return "Error: Failed to login to DeepSeek."

        try:
            # 2. Locate the input area with highly specific semantic selectors
            print("🔍 Searching for chat input...")
            # We try multiple selectors to be ultra-robust
            input_selectors = [
                'textarea[placeholder*="Message"]',
                'textarea',
                '[role="textbox"]:not([type="password"])',
                '#chat-input',
                '.chat-input-area'
            ]
            ...
            if not target_selector:
                raise Exception(f"Could not find any chat input selector after trying: {input_selectors}")

            # 3. Clear and Type
            print("⌨️ Typing prompt...")
            await self.page.fill(target_selector, "")
            await self._human_type(target_selector, prompt_text)
            ...
            while elapsed < max_wait:
                ...
                try:
                    # Check for "Stop" button (indicator of generation)
                    stop_btn = self.page.get_by_role("button", name="Stop")
                    is_generating = await stop_btn.is_visible()
                    
                    if is_generating:
                        print(f"🔄 Response still generating... ({elapsed}s)")
                        last_content = ""
                        stable_count = 0
                        continue
            ...
            print(f"⚠️ Response generation timed out after {max_wait}s. Returning best available: {last_content[:50]}")
