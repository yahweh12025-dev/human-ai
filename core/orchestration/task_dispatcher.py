...
class TaskDispatcher:
    ...
    async def dispatch(self, task: Dict[str, Any]):
        ...
        if any(kw in description for kw in worker_keywords):
            print(f"🚀 Dispatching to OpenCode (Primary Worker): {description[:50]}...")
            # Routing now goes directly to OpenCode via the subagent spawn system
            # Since OpenCode is spawned as a sub-agent, we return a routing signal
            return {"status": "routed", "target": "OpenCode", "task": task['description']}
