...
GSD is a Claude Code meta-prompting system installed at:
  ~/.claude/get-shit-done/            — runtime workflows
  ~/.claude/skills/gsd-*/            — individual skill modules
...
_GSD_INSTALL_DIR = Path.home() / ".claude" / "get-shit-done"
_GSD_SKILLS_DIR  = Path.home() / ".claude" / "skills"
_LOG_DIR         = _PROJECT_ROOT / "data" / "logs"

# ── GSD skill name → human description mapping ────────────────────────────────
# Subset relevant to the human-ai swarm. Full list: ~/.claude/skills/gsd-*/
...
def is_gsd_installed() -> bool:
    """Return True if GSD runtime is present in ~/.claude/get-shit-done/."""
    return (_GSD_INSTALL_DIR / "VERSION").exists()


def list_available_skills() -> list[str]:
    """Return sorted list of gsd-* skill names installed in ~/.claude/skills/."""
...
def invoke_gsd_skill(
    ...
    if not is_gsd_installed():
        msg = (
            "GSD runtime not found at ~/.claude/get-shit-done/. "
            "Install via: cd ~/.claude && npm install get-shit-done"
        )
        result["error"] = msg
