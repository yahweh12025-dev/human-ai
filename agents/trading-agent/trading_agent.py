class TradingAgent:
    def __init__(self):
        """Initialize the TradingAgent with all required components."""
        # Import and initialize components here to avoid circular imports
        from agents.trading_agent.data_fetcher import DataFetcher
        from agents.trading_agent.risk_manager import RiskManager
        from agents.trading_agent.signal_intelligence import SignalIntelligence
        from agents.trading_agent.market_intel import MarketIntelligence
        from agents.trading_agent.execution.paper_trader import PaperTrader
        ...
        def update_market_intelligence(self):
            ...
            try:
                ...
                if articles:
                    ...
                    # Store intelligence for use in symbol processing
                    self.latest_market_intel = {
                        'articles': articles,
            ...
            try:
                ...
                if symbol not in self.risk_manager.open_positions:
                    ...
                    if signal != 0:
                        ...
                        self.logger.debug(f"No trading signal for {symbol} after market intelligence filter")
                else:
        ...
        def _apply_market_intelligence_filter(self, symbol: str, original_signal: int) -> int:
            ...
            # Get sentiment for this symbol (default to 0 if not found)
            symbol_sentiment = sentiment_scores.get(symbol, 0)
        
            # If sentiment is strongly negative and we have a buy signal, consider filtering it out
            if original_signal > 0 and symbol_sentiment < -0.3:  # Strong negative sentiment
                self.logger.info(f"Filtering BUY signal for {symbol} due to negative sentiment: {symbol_sentiment:.3f}")
                return 0  # Neutralize the signal
            
            # If sentiment is strongly positive and we have a sell signal, consider filtering it out
            if original_signal < 0 and symbol_sentiment > 0.3:  # Strong positive sentiment
        ...
        def _apply_market_intelligence_sizing(self, symbol: str, original_quantity: float) -> float:
            ...
            # Positive sentiment increases size, negative sentiment decreases size
            # Range: -1 to 1 sentiment -> 0.5 to 1.5 multiplier
            sentiment_multiplier = 0.5 + (symbol_sentiment + 1) * 0.5  # Maps [-1,1] to [0.5, 1.5]
        
            # Limit the multiplier to reasonable bounds
