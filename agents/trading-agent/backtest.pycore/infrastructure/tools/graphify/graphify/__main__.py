...
_SKILL_REGISTRATION = (
    "\n# graphify\n"
    "- **graphify** (`~/.opencode/skills/graphify/SKILL.md`) "
...
_PLATFORM_CONFIG: dict[str, dict] = {
    "claude": {
        "skill_file": "skill.md",
        "skill_dst": Path(".opencode") / "skills" / "graphify" / "SKILL.md",
        "claude_md": True,
    },
    "codex": {
...
def install(platform: str = "claude") -> None:
    ...
    if cfg["claude_md"]:
        # Register in ~/.opencode/CLAUDE.md (opencode only)
        claude_md = Path.home() / ".opencode" / "CLAUDE.md"
        if claude_md.exists():
...
def claude_install(project_dir: Path | None = None) -> None:
    ...
    # Also write opencode PreToolUse hook to .opencode/settings.json
    _install_opencode_hook(project_dir or Path("."))

    print()
    print("OpenCode will now check the knowledge graph before answering")
    print("codebase questions and rebuild it after code changes.")


def _install_opencode_hook(project_dir: Path) -> None:
    """Add graphify PreToolUse hook to project .opencode/settings.json."""
    ...
    settings_path.write_text(json.dumps(settings, indent=2), encoding="utf-8")
    print(f"  .opencode/settings.json  ->  PreToolUse hook registered")


def _uninstall_opencode_hook(project_dir: Path) -> None:
    """Remove graphify PreToolUse hook from project .opencode/settings.json."""
    settings_path = project_dir / ".claude" / "settings.json"
    if not settings_path.exists():
        return
    try:
    ...
    settings_path.write_text(json.dumps(settings, indent=2), encoding="utf-8")
    print(f"  .opencode/settings.json  ->  PreToolUse hook removed")
...
def claude_uninstall(project_dir: Path | None = None) -> None:
    ...
    _uninstall_opencode_hook(project_dir or Path("."))
