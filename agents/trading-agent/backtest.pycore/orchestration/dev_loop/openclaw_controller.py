...
class OpenClawController:
    ...
    def execute_task(self, task_id: str) -> bool:
        ...
        try:
            # Handle CrewAI workflow tasks differently
            if task.task_type == OpenClawTaskType.CREW:
                print(f"   Executing CrewAI workflow: {task.prompt}")
                # Run the CrewAI workflow skill
                cmd = [
                    'python3',
                    '/home/yahwehatwork/human-ai/agents/crewai_workflows/skills/run_crew_workflow.py',
                    task.prompt  # This should be the path to the workflow definition JSON
                ]

                # Add context as environment variables or additional args if needed
                if task.context:
                    # Pass context as JSON in an environment variable
                    import os
                    env = os.environ.copy()
                    env['CREW_WORKFLOW_CONTEXT'] = json.dumps(task.context)
                else:
                    env = os.environ.copy()

                print(f"   Executing: python3 run_crew_workflow.py {task.prompt}")
