...
SESSION_REGIME_PARAMS = {
    ...
    "london":   {"tp_r": 1.2,  "sl_atr": 1.5, "timeout": 20, "trail_r": 0.6, "min_score": 5},
    "london_ny":{"tp_r": 1.5,  "sl_atr": 1.5, "timeout": 40, "trail_r": 0.6, "min_score": 5},
    "ny_close": {"tp_r": 1.0,  "sl_atr": 1.2, "timeout": 16, "trail_r": 0.6, "min_score": 6},
    "after_ny": {"tp_r": 1.0,  "sl_atr": 1.2, "timeout": 16, "trail_r": 0.6, "min_score": 5},
}
...
ATR_DEAD_THRESHOLD = {
    "XAUUSD": 0.60,   # was 0.001 — now properly calibrated
    "XAGUSD": 0.03,
    "EURUSD": 0.00008,
    "GBPUSD": 0.00010,
}
...
def compute_signal(prices: list, trend_15m: str = "NEUTRAL", h1_bias: str = "NEUTRAL",
    ...
    # v11: price momentum must CONFIRM EMA cross (3 consecutive same-direction candles)
    ...
    # RSI confirmation (+1)
    score_bull = bull_pillars + (1 if rsi_buy  and bull_pillars >= 2 else 0)
    score_bear = bear_pillars + (1 if rsi_sell and bear_pillars >= 2 else 0)
    ...
    if h1_bias == "BULL" and bull_pillars >= 2: score_bull += 1
    if h1_bias == "BEAR" and bear_pillars >= 2: score_bear += 1

    score     = max(score_bull, score_bear)
    direction = "NONE"
    if score_bull >= 3 and score_bull > score_bear and rsi_buy:
        direction = "BUY"
    elif score_bear >= 3 and score_bear > score_bull and rsi_sell:
        direction = "SELL"
        ...
        if now - self.klines_ts[symbol] > 120 or not self.klines_1m[symbol]:
            ...
            if fresh:
                ...
                if live_price > 0:
                    ...
                    noise = (0.5 if symbol == "XAUUSD" else 0.05 if symbol == "XAGUSD" else 0.0003)
